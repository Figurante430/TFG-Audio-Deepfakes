...
OUTPUT_DIR = BUILD_DIR / "confucius_outputs"
GENERATION_CSV = BUILD_DIR / "confucius_generation.csv"
GENERATION_MANIFEST = BUILD_DIR / "confucius_generation_manifest.json"
JOBS_CSV = BUILD_DIR / "confucius_jobs.csv"

FINAL_ROOT = ROOT / "external_final_fake"
FINAL_DIR = FINAL_ROOT / "fake" / "confucius"

METADATA_CSV = FINAL_ROOT / "metadata_confucius.csv"
FREEZE_MANIFEST = FINAL_ROOT / "freeze_manifest_confucius.json"

EXPECTED_COUNT = 20
EXPECTED_SPEAKERS = 10
EXPECTED_SR = 22050

EXPECTED_GENERATOR = "Confucius4-TTS"
...
if gen_manifest.get("generator") != EXPECTED_GENERATOR:
    fail(
        f"Generator inesperado: "
...
if int(gen_manifest.get("sample_rate")) != EXPECTED_SR:
    fail(
        f"Sample rate inesperado: "
        f"{gen_manifest.get('sample_rate')}"
    )


if gen_manifest.get("reference_transcript_used") is not False:
    fail(
...
if len(rows) != EXPECTED_COUNT:
    fail(
        f"confucius_generation.csv tiene "
        f"{len(rows)} filas."
    )
...
for row in rows:
    ...
    if current_hash != row["sha256"].strip():
        fail(
    ...
    if int(row["sample_rate"]) != info["sample_rate"]:
        fail(
...
for i, item in enumerate(
    ...
    metadata_rows.append({
        ...
        # Conservada como metadata,
        # no usada por Confucius4
        "reference_text":
            row["reference_text"],
...
freeze_manifest = {
    ...
    "name":
        "External final fake holdout - Confucius4-TTS",
...
print()
print("=" * 72)
print("CONFUCIUS4 EXTERNAL FINAL HOLDOUT — CONGELADO")
print("=" * 72)

print(f"Generator : {EXPECTED_GENERATOR}")
