...
OUTPUT_DIR = BUILD_DIR / "openvoice_outputs"
GENERATION_CSV = BUILD_DIR / "openvoice_generation.csv"
GENERATION_MANIFEST = BUILD_DIR / "openvoice_generation_manifest.json"
JOBS_CSV = BUILD_DIR / "openvoice_jobs.csv"

FINAL_ROOT = ROOT / "external_final_fake"
FINAL_DIR = FINAL_ROOT / "fake" / "openvoice"

METADATA_CSV = FINAL_ROOT / "metadata_openvoice.csv"
FREEZE_MANIFEST = FINAL_ROOT / "freeze_manifest_openvoice.json"
...
EXPECTED_GENERATOR = "OpenVoice V2"
EXPECTED_BASE_GENERATOR = "MeloTTS"
...
base_tts = (
    gen_manifest.get("base_tts")
    or {}
)


if (
    base_tts.get("generator")
    != EXPECTED_BASE_GENERATOR
):
    fail(
        "Base TTS inesperado: "
        f"{base_tts.get('generator')}"
    )


if (
    base_tts.get("language")
    != EXPECTED_LANGUAGE
):
    fail(
        "Idioma base inesperado: "
        f"{base_tts.get('language')}"
    )


converter = (
    gen_manifest.get("converter")
    or {}
)


if not converter.get(
    "checkpoint_sha256"
):
    fail(
        "El manifest no contiene hash "
        "del checkpoint de OpenVoice."
    )
...
if len(rows) != EXPECTED_COUNT:
    fail(
        f"openvoice_generation.csv tiene "
        f"{len(rows)} filas."
    )
...
for row in rows:
    ...
    if (
        row["base_generator"].strip()
        != EXPECTED_BASE_GENERATOR
    ):
        fail(
            f"{row['id']}: base_generator incorrecto."
        )
...
for i, item in enumerate(
    ...
    metadata_rows.append({
        ...
        # Metadata únicamente.
        # No se utilizó para generar OpenVoice.
        "reference_text":
            row["reference_text"],
...
freeze_manifest = {
    ...
    "name":
        "External final fake holdout - OpenVoice V2",
    ...
    "model_identity": {
        ...
        "converter_config_path_at_generation":
            converter.get(
                "config"
            ),
        ...
        "converter_checkpoint_path_at_generation":
            converter.get(
                "checkpoint"
            ),

        "converter_checkpoint_sha256":
            converter.get(
                "checkpoint_sha256"
            ),
...
print()
print("=" * 72)
print("OPENVOICE V2 EXTERNAL FINAL HOLDOUT — CONGELADO")
print("=" * 72)
