...
OUTPUT_DIR = BUILD_DIR / "cosyvoice_outputs"
GENERATION_CSV = BUILD_DIR / "cosyvoice_generation.csv"
GENERATION_MANIFEST = BUILD_DIR / "cosyvoice_generation_manifest.json"
JOBS_CSV = BUILD_DIR / "cosyvoice_jobs.csv"

FINAL_ROOT = ROOT / "external_final_fake"
FINAL_DIR = FINAL_ROOT / "fake" / "cosyvoice"

METADATA_CSV = FINAL_ROOT / "metadata_cosyvoice.csv"
FREEZE_MANIFEST = FINAL_ROOT / "freeze_manifest_cosyvoice.json"

EXPECTED_COUNT = 20
EXPECTED_SPEAKERS = 10
EXPECTED_SR = 24000

EXPECTED_GENERATOR = "Fun-CosyVoice3"
EXPECTED_MODEL = "Fun-CosyVoice3-0.5B"
...
if gen_manifest.get("model") != EXPECTED_MODEL:
    fail(
        "Modelo inesperado:\n"
        f"{gen_manifest.get('model')}"
    )
...
if len(generated_rows) != EXPECTED_COUNT:
    fail(
        f"cosyvoice_generation.csv tiene "
...
for row in generated_rows:
    ...
    if csv_sr != info["sample_rate"]:
        fail(
            f"{filename}: sample rate del CSV "
            "no coincide."
        )
...
freeze_manifest = {
    ...
    "name":
        "External final fake holdout - Fun-CosyVoice3",
...
print()
print("=" * 72)
print("COSYVOICE EXTERNAL FINAL HOLDOUT — CONGELADO")
print("=" * 72)
