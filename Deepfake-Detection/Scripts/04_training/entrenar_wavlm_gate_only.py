...
# ÚNICA AUGMENTATION DEL EXPERIMENTO
GATE_PROB = 0.50
GATE_DBFS_MIN = -55.0
GATE_DBFS_MAX = -35.0
...
def random_gate(x):
    """
    Única augmentation del experimento:
    con probabilidad GATE_PROB, pone a cero muestras cuya amplitud
    absoluta esté por debajo de un umbral aleatorio entre -55 y -35 dBFS.

    Se aplica SOLO a train y de forma idéntica a real/fake.
    No cambia duración, sample-rate, alineamiento ni volumen global.
    """
...
class AudioDataset(Dataset):
    def __init__(self, rows, root: Path, split: str, mode: str):
        self.rows = list(rows)
        self.root = root
        self.split = split
        self.mode = mode
    ...
    def __getitem__(self, idx):
        row = self.rows[idx]
        path = resolve_audio_path(self.root, row["path"])
        ...
        # SOLO TRAIN, y SOLO en gate_only.
        if (
            self.split == "train"
            and self.mode == "gate_only"
        ):
            x = random_gate(x)
...
def unfreeze_last_layers(model, n_layers=LAST_N_LAYERS):
    for p in model.wavlm.parameters():
        p.requires_grad = False

    for layer in model.wavlm.encoder.layers[-n_layers:]:
...
def train_run(root: Path, rows, seed: int, force=False):
    mode = "gate_only"
    ...
    if (
        ...
        print(
            f"[SKIP] gate_only seed={seed}: ya terminado."
        )
        return
    ...
    print(
        f"gate_prob={GATE_PROB} "
        f"threshold_dbfs=[{GATE_DBFS_MIN}, {GATE_DBFS_MAX}]"
    )
    ...
    result = {
        ...
        "augmentation": {
            ...
            "type": "random_gate_only",
            "probability": GATE_PROB,
            "threshold_dbfs_min": GATE_DBFS_MIN,
            "threshold_dbfs_max": GATE_DBFS_MAX,
        },
        "history": history,
    }
